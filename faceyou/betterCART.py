...
如下一个样本集：
dataset_list = [ ['youth', 'work_no', 'house_no', '1', 'refuse']
...
class CCartTree(object):
    def __init__(self, dataset, labels, div_label, max_n_feats):
    ...
    def work(self, dataset, labels, div_label, max_n_feats):
        '''
        给定样本数据集+特征列表，找出最优特征，最优切分点，最优叶节点，次优切分点
        :param dataset:样本数据集
        :param labels:特征列表, 比如['age','work','house','credit']
        :return 样本集的最优特征，最优切分点最优叶节点
        :Note,每个样本=[ageVal, workVal, houseVal, creditVal, classLabel]
        '''
        stat, n_dataset = {}, len(dataset)
        # 获得标签列表并去重
        class_vals = [e[-1] for e in dataset]
        class_set = set(class_vals)
        
        for i in range(len(labels)):
            f, stat[f] = labels[i], {}  # feature
            for e in dataset:
                # v为某一特征的值， c为该数据对应的分类(标签)
                v, c = e[i], e[-1]
                if v not in stat[f].keys():
                    ...
                    stat[f][v][c], stat[f][v][c]['n'], stat[f][v][c]['p'] = {}, 0, 0.0
                stat[f][v]['n'] += 1
                stat[f][v]['p'] = stat[f][v]['n'] / n_dataset
                stat[f][v][c]['n'] += 1
                ...
                prob = (class_vals.count(div_label) - stat[f][v][div_label]['n']) / (n_dataset - stat[f][v]['n'])
                d1_gini, d2_gini = stat[f][v][div_label]['gini'], 2 * prob * (1 - prob)
                stat[f][v]['gini'] = d1_p * d1_gini + d2_p * d2_gini
        # 选取最优特征，最优切分点, 最优叶子节点
        min_v_gini, bf_bv = 9527, []
        for i in range(len(labels)):
            f = labels[i]  # visit every feature
            for v in set([e[i] for e in dataset]):  # visit every value of feature
        ...
        bf_v_set = set([e[labels.index(bf)] for e in dataset])
        bf_v_set.remove(bv)
    ...
    def create_tree(self):
        if len(self.labels) < self.max_n_feats:
            return None
        # get current tree
        bf, bv, bc, better_v, better_c, stat = self.work(self.dataset,
                                                         self.labels,
                                                         self.div_label,
                                                         self.max_n_feats)
        ...
        tailor = CTailordataset(self.dataset,
                                self.labels,
                                self.labels.index(bf),
                                bv)
        new_dataset, new_labels = tailor.get_dataset()

        cart = CCartTree(new_dataset,
                         new_labels,
                         self.div_label,
                         self.max_n_feats)
...
if __name__ == '__main__':
    data_list, labels = initdataSet()
    # 由CART算法得到决策树字典
    cart = CCartTree(data_list, labels, 'agree', 3)
